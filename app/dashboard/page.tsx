...
export default function DashboardPage() {
  ...
  const analytics = [
    ...
    },
    {
      ...
      sub: "Fully processed",
      change: total > 0 ? `${Math.round((completed / total) * 100)}% completion rate` : "0%",
      icon: CheckCircle2,
      accent: "#16A34A",
      bg: "#F0FDF4",
    ...
    <main className="min-h-screen p-6 sm:p-8" style={{ backgroundColor: "#F7F7F7" }}>
        ...
        {analytics.map(({ label, value, sub, change, icon: Icon, accent, bg, bar }) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <span className="flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold" style={{ backgroundColor: bg, color: accent }}>
            ...
            <div className="mt-4 h-1.5 w-full rounded-full" style={{ backgroundColor: "#F3F4F6" }}>
              <div className="h-1.5 rounded-full transition-all duration-500" style={{ width: `${bar}%`, backgroundColor: accent }} />
            </div>
            <div className="mt-2 flex items-center gap-1">
              <ArrowUpRight className="h-3 w-3" style={{ color: accent }} />
              <span className="text-xs font-medium" style={{ color: accent }}>{change}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      >
        ...
        <div
          ...
          style={{ gridTemplateColumns: "1fr 2fr 1fr 1fr 1fr", backgroundColor: "#F9FAFB", color: "#9CA3AF", borderBottom: "1px solid #F3F4F6" }}
        >
          <span>Tracking #</span>
          <span>Title</span>
          <span>Type</span>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-sm" style={{ color: "#9CA3AF" }}>No documents found.</div>
        ) : (
          documents.map((doc, i) => {
            ...
            return (
              <div
                ...
                style={{ gridTemplateColumns: "1fr 2fr 1fr 1fr 1fr", borderBottom: i < documents.length - 1 ? "1px solid #F3F4F6" : "none" }}
              >
